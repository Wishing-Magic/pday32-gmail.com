...
class steam(commands.Cog):
    ...
    @commands.command()
    async def steamuserinfo(self, ctx: Context, vanityurl):
        """Get information on a user (requires VanityURl)"""

        message = await ctx.send("Getting Information...")
        # Convert VanityURL to SteamID64
        steamid = await Utils.vanitytosteamid(vanityurl)
        # Get player Info
        userinfo = requests.get("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=FF0EEF99E5BD63F29FC0F938A56F115C&steamids=" + str(steamid))
        ...
        embed.add_field(name="Last logoff", value=userjson['lastlogoff'])
        await ctx.send("User Information Found!", embed=embed)

        await ctx.author.send("```"+str(userjson)+"```")

    @commands.command()
    async def vacban(self, ctx: Context, user):
